...
class ProductSelect(ui.Select):
    def __init__(self, db_manager):
        ...
        options = [
            ...
            for product in PRODUCTS  # Используем PRODUCTS из config.py
        ]
        super().__init__(placeholder="Выберите товар", min_values=1, max_values=1, options=options)

    async def callback(self, interaction: Interaction):
        selected_product_name = self.values[0]
        selected_product = next((product for product in PRODUCTS if product["name"] == selected_product_name), None)  # Получаем товар из PRODUCTS
...
class ConfirmPurchaseButton(ui.Button):
    ...
    async def callback(self, interaction: Interaction):
        ...
        if user and user.get("tokens", 0) >= self.product["cost"]:
            ...
            await interaction.response.send_message("Покупка успешно выполнена!", ephemeral=True)
        else:
            await interaction.response.send_message("Недостаточно токенов для покупки.", ephemeral=True)
