...
def send_api_request(prompt: str) -> str:
    from_prompt = "Я буду предоставлять вам ролевые истории, отправленные пользователями для Minecraft RP сервера DiceRP(Дайс). Ваша задача - отвечать 'true' или 'false'. Отвечайте 'true', если история приемлема, и 'false', если нет. Приемлемая история должна описывать вымышленного персонажа или персонажа, созданного по мотивам известных вселенных (мение желательно), содержать описание его характеристик (минимальные хотя бы, не критично если их почти нету), быть без грубых грамматических ошибок и не содержать неприемлемого контента. История может быть умористической но главное чтобы был не перебор. Желательно, чтобы история была детализированной, с логичным сюжетом и демонстрировала развитие персонажа, но это не обязательно. Небольшие 'мета-ссылки' допустимы, но чрезмерное их использование нежелательно. Неприемлемы истории о реальных игроках, с недостаточным описанием персонажа, созданные за короткое время или бессмысленные. не состиковки в сюжете истории не критичные. Слишком быстрое развитие событий не является критичным фактором. Соответствие сеттингу Minecraft RP сервера DiceRP(Дайс) является желательным, но не обязательным фактором. Обращайте внимание на описание персонажа, так как истории с недостаточной детализацией менее предпочтительны. Вот история:" + prompt
    headers = {
